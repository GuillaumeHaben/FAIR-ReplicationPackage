...
# Some parameters for the program (See README)
saveArtifacts = True
notification = False
savePassTests = False
...
def queryTestVariants(invocation, pageToken):
    ...
    for i in range(5):
        try:
            response = requests.post('https://results.api.cr.dev/prpc/luci.resultdb.internal.ui.UI/QueryTestVariants', headers=headers, data=data)
            if response.status_code != 200:
                print("Error in the response:", response.status_code)
