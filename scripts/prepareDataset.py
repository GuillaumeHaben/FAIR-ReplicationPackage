...
def main():
        ...
        if os.path.isdir(buildDir):
            ...
            with open(buildDir.path + "/testsInfo.json") as testsInfo:
                ...
                # TEST STATUS
                    ...
                    elif test["status"] == "EXONERATED":
                        testStatus = 1
                    elif test["status"] == "EXPECTED":
                        testStatus = 2
                    elif test["status"] == "UNEXPECTED":
                        testStatus = 3
                        # History of failure
                        heartBeat[buildDir.name].append(testId)
                    elif test["status"] == "UNEXPECTEDLY_SKIPPED":
                        testStatus = 4
                    else:
                        print("Unhandle Test Status", test["status"])
                        sys.exit(1)
                ...
                # RUNS
                    for run in test["results"]:
                        ...
                        if runTagStatus == "CRASH":
                            ...
                            sys.exit(1)
                        # Flaky
                        if testStatus == 0 and runStatus != 2:
                            label = 0
                        # Failure
                        elif testStatus == 3 and runTagStatus != 1:
                            label = 1
    ...
    for element in dataset:
        ...
        if pastOnly == True:
            buildsToConsiderLinux = int(buildId) >= 97983 + heartBeatWindowSize + 1
        ...
        if buildsToConsiderLinux and datasetName == "Linux" or buildsToConsiderMac and datasetName == "Mac"  or buildsToConsiderWindows and datasetName == "Windows":
            statusList = []
            statusListFlaky = []
